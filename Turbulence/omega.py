...
# Compute spectrogram
stft = signal.ShortTimeFFT(win, hop=1, fs=1/sampling_period,phase_shift=0)
...
ax.set_xlabel(r"$b$")
ax.set_ylabel(r"$k$")
ax.set_title("Wave-number-normalized spectrogram \n" + r"$P_{S} \omega_x(k,b) / \max_b(P_{S} \omega_x(k,b))$")
fig.colorbar(pcm, ax=ax)
plt.tight_layout()
fig.savefig('./result/turbulence/omega_spectro.jpg')
plt.close(fig)
...
ax2.set_title("STFT Phase \n" + r"$\angle STFT_{\omega_x}(k,b)$")
fig2.colorbar(pcm2, ax=ax2)
plt.tight_layout()
fig2.savefig('./result/turbulence/omega_spectro_phase.jpg')
plt.close(fig2)
...
ax.set_yscale("log")
ax.set_xlabel(r"$k$")
ax.set_title("Spectrogram energy per wavenumber\n" + r"$\sum_b P_{S} \omega_x(k,b)$")
plt.tight_layout()
fig.savefig('./result/turbulence/omega_spectro_energy.jpg')
plt.close(fig)
...
ax1.set_xlabel(r"$x$")
ax1.set_ylabel(r"$k$")
ax1.set_title("Wave-number-normalized scalogram \n" + r"$P_{W} \omega_x(k,b) / \max_b(P_{S} \omega_x(k,b))$")
fig1.colorbar(pcm1, ax=ax1)
plt.tight_layout()
fig1.savefig('./result/turbulence/omega_cwt.jpg')
plt.close(fig1)
...
ax2.set_xlabel(r"$x$")
ax2.set_ylabel(r"$k$")
ax2.set_title("CWT Phase \n" + r"$\angle CWT_{\omega_x}(k,b)$")
fig2.colorbar(pcm2, ax=ax2)
plt.tight_layout()
fig2.savefig('./result/turbulence/omega_cwt_phase.jpg')
plt.close(fig2)
...
ax.set_title("Scalogram energy per wavenumber\n" + r"$\sum_b P_{W} \omega_x(k,b)$")
plt.tight_layout()
fig.savefig('./result/turbulence/omega_cwt_energy.jpg')
plt.close(fig)
...
ax.set_ylabel("Energy")
plt.tight_layout()
fig.savefig('./result/turbulence/omega_fft.jpg')